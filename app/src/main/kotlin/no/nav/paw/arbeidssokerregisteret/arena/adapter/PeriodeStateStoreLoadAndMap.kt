...
class PeriodeStateStoreLoadAndMap(
    ...
    private fun process(
        ...
        if (periode != null) {
            val arenaTilstand = byggArenaTilstand(record.key(), periode, record.value().first, record.value().second)
            context.forward(record.withValue(arenaTilstand))
            db.delete(record.key())
        } else {
            logger.debug("Ignorerte key={}, opplysningsId={}, profileringsId={}, grunnet manglende periode(id={})",
                record.key(),
...
fun byggArenaTilstand(
    key: Long?,
    periode: Periode,
    first: OpplysningerOmArbeidssoeker,
    second: Profilering
