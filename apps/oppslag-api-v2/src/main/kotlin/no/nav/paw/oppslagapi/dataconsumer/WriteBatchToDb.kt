...
import no.nav.paw.oppslagapi.dataconsumer.DataTable
import no.nav.paw.oppslagapi.dataconsumer.Row
...
fun writeBatchToDb(rows: Sequence<Pair<Row<String>, Span>>) {
    val tracer = GlobalOpenTelemetry
        .getTracer("paw-arbeidssoekerregisteret-api-oppslag-v2")
    val batchSpan = tracer.spanBuilder("writeBatchToDb")
        .setSpanKind(SpanKind.SERVER)
        .startSpan()
    val linkedRows = rows
        .map { (row, span) ->
            val span = span
                .addLink(batchSpan.spanContext)
                .addEvent("added_to_batch")
            try {
                row
            } finally {
               span.end()
